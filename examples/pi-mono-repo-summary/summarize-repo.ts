...
const concurrency = Number(process.env.PI_MONO_REPO_SUMMARY_CONCURRENCY ?? "4");
...
if (!Number.isFinite(maxBytes) || maxBytes <= 0) {
  throw new Error("PI_MONO_REPO_SUMMARY_MAX_BYTES must be a positive number.");
}

if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
  throw new Error("PI_MONO_REPO_SUMMARY_TIMEOUT_MS must be a positive number.");
}

if (!Number.isFinite(concurrency) || concurrency <= 0) {
  throw new Error("PI_MONO_REPO_SUMMARY_CONCURRENCY must be a positive number.");
}

await mkdir(runDir, { recursive: true });

// Reuse pi's local OAuth storage when available so the example works with existing pi logins.
...
// Parse GitHub tree URLs so a link like /tree/main/packages/ai summarizes only the requested subtree.
...
// Walk the tree iteratively so the whole repository is mapped before summarization starts.
...
for (const relativePath of mappedFiles) {
  ...
  if (lowSignalFileNames.has(baseName) || relativePath.includes("/generated/")) {
...
if (!apiKey && authData[provider]) {
  ...
  if (oauthResult?.apiKey) {
    ...
    authData[provider] = { type: "oauth", ...oauthResult.newCredentials };
    await writeFile(authFile, `${JSON.stringify(authData, null, 2)}\n`).catch(() => {});
  }
}

const callModel = async (prompt: string) => {
  // Use one direct completion per node so every merge step stays independent.
    ...
    const response = await Promise.race([
      ...
      new Promise<never>((_, rejectPromise) => {
        timeoutHandle = setTimeout(
          () => rejectPromise(new Error(`Model call timed out after ${timeoutMs}ms`)),
          timeoutMs,
        );
      }),
    ]);
...
const finalSummary = await callModel(finalPrompt);
