...
// Support GitHub tree URLs because many summary examples start from a shared link.
...
for (let index = 0; index < fileEntries.length; index += 2) {
  ...
  const summary = await completeWithTimeout(client, prompt, timeoutMs);
  summaryNodes.push({
    level: 0,
...
async function completeWithTimeout(client: CopilotClient, prompt: string, timeoutMs: number): Promise<string> {
    ...
    const response = await Promise.race([
      ...
      new Promise<never>((_, rejectPromise) => {
        timeoutHandle = setTimeout(
          () => rejectPromise(new Error(`Copilot SDK call timed out after ${timeoutMs}ms`)),
          timeoutMs,
        );
      }),
    ]);
