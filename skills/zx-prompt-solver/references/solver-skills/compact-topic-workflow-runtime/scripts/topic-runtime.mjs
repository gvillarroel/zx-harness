...
export async function runHarness({ harness, prompt, command }) {
  // Read raw argv so option-like and shell-shaped topics remain data; only the exact flag is control.
  const inputArgs = process.argv.slice(3);
  const dryRun = inputArgs.includes("--dry-run") || process.env.TOPIC_DRY_RUN === "1";
  const topic = inputArgs.filter((value) => value !== "--dry-run").join(" ").trim();
